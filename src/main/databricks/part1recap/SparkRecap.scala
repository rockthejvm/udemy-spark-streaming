...
// MAGIC %md
// MAGIC # Preface
// MAGIC This notebook is set up to read the data files from an Azure Blob Container.
// MAGIC The following two cells exist to setup that connection.
// MAGIC The value `strgDir` needs to be used in place of the path `src/main/resources/data` from the original scripts for this and all following notebooks.
...
// // This cell initializes the Spark Structured API
...
val usefulCarsData = cars.select(
  ...
  expr("Weight_in_lbs / 2.2").as("Weight_in_kg2") // same as above using a SQL expression instead of a scala one
)
